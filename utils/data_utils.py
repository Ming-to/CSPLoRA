...
from utils.offline_hf import load_hf_dataset_offline
...
def load_and_preprocess_data(task, tokenizer, args):
    if "mnli" in task:
        dataset = load_hf_dataset_offline("glue", "mnli")
    else:
        dataset = load_hf_dataset_offline("glue", task)
    ...
    if task == "cola":
        ...
        tokenized_datasets = tokenized_datasets.remove_columns(["sentence1", "sentence2", "idx"])
    elif task in ["mnli_matched", "mnli_mismatched", "mnli"]:
    ...
    if task in {"cola", "sst2", "mrpc", "qqp", "stsb", "qnli", "rte", "wnli"}:
        ...
        train_dataset = tokenized_datasets["train"]
        val_dataset = tokenized_datasets["validation_matched"]
        test_dataset = tokenized_datasets["validation_mismatched"]
    else:
        raise ValueError(f"Unexpected task {task}")
