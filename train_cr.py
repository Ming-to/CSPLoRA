...
from llama_args import get_cr_args
...
def get_run_dir_path(args):
    ...
    如果指定了 --exp_name，则使用简化路径:
    {output_dir}/{exp_name}/

    否则使用默认路径格式:
    {output_dir}/commonsense_reasoning/{model_name}/{method}_r{rank}_seed{seed}/
    例如: experiments_llama/commonsense_reasoning/Llama-3.1-8B/lora_r8_seed42/
    """
    # 如果指定了 exp_name，使用简化路径
    if getattr(args, "exp_name", None):
        return os.path.join(args.output_dir, args.exp_name)

    # 默认路径格式
    base_dir = os.path.join(args.output_dir, "commonsense_reasoning")
...
def finetune(args):
    ...
    if method == "full":
        ...
        if getattr(args, "csplora", False) or getattr(args, "csplora_ada", False):
            raise ValueError("[GoRA] method='gora' 与 --csplora/--csplora_ada 冲突：请二选一。")

        print("[GoRA] Initializing GoRA for task: commonsense")
        ...
        if args.csplora or getattr(args, "csplora_ada", False):
            ...
            if is_main_process:
                ...
                if use_ada:
                    ...
                    # Use custom cache_dir if provided, otherwise default
                    cache_dir = getattr(args, "csplora_cache_dir", None) or os.path.join(args.output_dir, "csplora_cache")
                    task_suffix = getattr(args, "csplora_task_suffix", "")
                    task_name = f"commonsense{task_suffix}"
        ...
        model, peft_config = create_peft_model_cr(
            model=model,
            args=args,
