...
def test_hendrycks_math(model, data_path, start=0, end=MAX_INT, batch_size=1, tensor_parallel_size=1):
    ...
    print(f'[INFO] Total MATH samples: {len(hendrycks_math_ins)}')
    hendrycks_math_ins = hendrycks_math_ins[start:end]
    hendrycks_math_answers = hendrycks_math_answers[start:end]
    print(f'[INFO] Selected samples: {len(hendrycks_math_ins)}')
    ...
    print(f'[RESULT] MATH length={len(results)}, accuracy={acc:.4f}')


def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--model", type=str, default=0)
    parser.add_argument("--data_file", type=str, default='data/math_eval/MATH_test.jsonl')
