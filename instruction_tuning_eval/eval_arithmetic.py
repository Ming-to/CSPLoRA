...
def load_model_and_tokenizer(base_model: str, ckpt_dir: str):
    """
    优先按“base_model + LoRA adapter”方式加载；
    如果 ckpt_dir 里是 full-finetune 的完整权重，则自动 fallback。
    """
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    # 1) tokenizer：优先用 ckpt_dir 下保存的；没有就回退到 base_model
    try:
    ...
    # 2) model：先尝试作为 LoRA adapter，失败再当完整模型加载
    try:
...
def evaluate_gsm8k(args):
    ...
    print(f"[RESULT] GSM8K accuracy = {acc:.4f}  ({correct}/{total})")
...
def evaluate_math(args):
    ...
    for ex in tqdm(ds, desc="MATH"):
        # 适配本地 JSONL 格式：instruction/output 而非 problem/solution
    ...
    acc = correct / max(total, 1)
    print(f"[RESULT] MATH accuracy (strict string match) = {acc:.4f}  ({correct}/{total})")
...
def evaluate_metamathqa(args):
    ...
    for ex in tqdm(ds, desc="MetaMathQA"):
        ...
        gen_ids = output_ids[0][inputs["input_ids"].shape[1]:]
        gen_text = tokenizer.decode(gen_ids, skip_special_tokens=True)
        is_correct = _numeric_match(gen_text, gold_raw)
    ...
    acc = correct / max(total, 1)
    print(f"[RESULT] MetaMathQA accuracy = {acc:.4f}  ({correct}/{total})")
