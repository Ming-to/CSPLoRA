# eval_arithmetic_batch.py
# 批量推理优化版 - 主要加速点：batch inference
import sys
import os
import json
import re
...
def load_model_and_tokenizer(base_model: str, ckpt_dir: str):
    """
    加载模型和tokenizer（与原版eval_arithmetic.py保持一致）
    """
    ...
    tokenizer.padding_side = "left"  # 批量推理需要 left padding
...
def get_model_metadata(ckpt_dir: str) -> dict:
    ...
    # Method 2: If we couldn't infer, try reading wandb_run_id.txt
...
def batch_generate(
    ...
    for i in tqdm(range(0, len(prompts), batch_size), desc="Batch generation"):
        ...
        # 批量 tokenize，padding 到同一长度
        inputs = tokenizer(
            batch_prompts,
            return_tensors="pt",
            padding=True,  # 关键：padding
            truncation=True,
            max_length=2048,
        )
        ...
        # 解码每个生成的文本
        # model.generate() 返回 [batch_size, padded_input_len + generated_len]
        # padding 会被保留在输出中，所以使用统一的 padded_length 来截取
...
if __name__ == "__main__":
    ...
    if not hasattr(args, 'batch_size'):
        args.batch_size = 16  # 默认 batch size
