...
# 评测版本选择（三选一，优先级：BATCH > VLLM > 原版）
USE_BATCH=1      # 使用批量推理版本（4-5x加速，准确率随机波动<1%）
USE_VLLM=0       # 使用vLLM版本（最快，但准确率可能有0-0.3%差异）
BATCH_SIZE=8     # 批量推理的batch size（建议8，不要超过8以避免准确率下降）
...
# LoRA 方法和配置
# METHODS=("full" "adalora" "gora" "lora" "pissa" "lora_plus")
METHODS=("lora" "pissa" "lora_plus")
RANK=8
...
# 选择评测脚本（优先级：BATCH > VLLM > 原版）
if [ "${USE_BATCH}" = "1" ]; then
  EVAL_SCRIPT="instruction_tuning_eval/eval_arithmetic_batch.py"
  EXTRA_ARGS="--batch_size ${BATCH_SIZE}"
  echo "Using Batch Inference version: ${EVAL_SCRIPT} (batch_size=${BATCH_SIZE})"
  echo "  → Expected speedup: 4-5x"
  echo "  → Accuracy: Random fluctuation <1%, same as different seed"
elif [ "${USE_VLLM}" = "1" ]; then
  EVAL_SCRIPT="instruction_tuning_eval/eval_arithmetic_vllm.py"
  EXTRA_ARGS=""
  echo "Using vLLM version: ${EVAL_SCRIPT}"
  echo "  → Expected speedup: 10-20x"
  echo "  → Accuracy: Possible difference 0-0.3%"
else
